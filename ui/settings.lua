...
function Settings.enter()
  Settings.items = {
    { label = "Lautstärke", key = "masterVolume", type = "slider", min = 0, max = 100, value = Config.masterVolume or 80 },
    { label = "Musik", key = "musicVolume", type = "slider", min = 0, max = 100, value = Config.musicVolume or 70 },
    { label = "SFX", key = "sfxVolume", type = "slider", min = 0, max = 100, value = Config.sfxVolume or 80 },
    { label = "FPS anzeigen", key = "showFPS", type = "toggle", value = Config.showFPS or false },
    { label = "Vollbild", key = "fullscreen", type = "toggle", value = Config.fullscreen or false },
  }
  Settings.selected = 1
end
...
function Settings.draw()
  ...
  Components.drawPanel(w * 0.3, h * 0.15, w * 0.4, h * 0.5, "Einstellungen")
  ...
  for i, item in ipairs(Settings.items) do
    ...
    elseif item.type == "toggle" then
      love.graphics.printf(item.value and "AN" or "AUS", px + 300, py, 50, "left")
    end
    py = py + 35
  end
  Components.drawButton("Zurück", w * 0.45, h * 0.7, 150, 35, false)
end
